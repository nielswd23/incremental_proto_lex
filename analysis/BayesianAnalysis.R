...
fit_and_predict_bayesian <- function(train_dataset, seed, gram, model){
  ...
  b_model <- brm(my_bf,
                 ...
                 thin = 1,
                 seed = seed,
                 save_model = paste0("./KFoldModelFits_positional_incremental/",gram,"_prob_",model,"_brmsSeed_",seed,".stan"),
                 file = paste0("./KFoldModelFits_positional_incremental/",gram,"_prob_",model,"_brmsSeed_",seed,"fit"),
...
for (model in list_of_model_types[c(7,8)]) {
  ...
  dir.create("./Results_incremental/AGSimple")
  write_csv(res, paste0("./Results_incremental/AGSimple/", model, "_auto_kfold.csv"))
}
